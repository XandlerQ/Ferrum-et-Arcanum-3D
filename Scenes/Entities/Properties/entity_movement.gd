...
# Jump speed
@export var jumpSpeed: float = 4.5;
# Terminal fall speed
...
# Maximum horisontal speed
var maxHorizontalSpeed: float = max(walkSpeed, runSpeed, dashSpeed);
...
func walk_in_direction(delta: float, direction: Vector2) -> void:
	if (direction == Vector2.ZERO):
		return stop(delta);
	var rVelocity: Vector2 = VectorConverter.convert_vector_3_2(entity.velocity);
...
func dash_in_direction(delta: float, direction: Vector2) -> void:
	if (direction == Vector2.ZERO or !dashTimer.is_stopped()):
		return stop(delta);
	dashTimer.start();
	var rVelocity: Vector2 = VectorConverter.convert_vector_3_2(entity.velocity) / 2;
